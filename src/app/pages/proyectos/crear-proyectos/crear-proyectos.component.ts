...
export class CrearProyectosComponent implements OnInit, OnDestroy {
  ...
  public crearProyecto = () =>{
    ...
    this.fileUploadSrv.uploadImageServices(this.imgUpload).then( resp =>{
      this.formCrearProyecto.patchValue({['imagen']: resp.nombreImg });
      
      this.proyectosSrv.insertProjectService(this.formCrearProyecto.value).pipe(takeUntil(this._unsubscribeAll)).subscribe( (resp:any) =>{
